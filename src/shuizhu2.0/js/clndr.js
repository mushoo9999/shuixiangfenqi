...
(function (factory) {
    ...
    var clndrTemplate =
        "<div class='clndr-controls'>" +
            "<div class='clndr-topbar'>"+
                "<div class='clndr-tag clearfix'>" +
                    "<span class='clndr-huan'>还款日</span>" +
                    "<span class='clndr-hui'>回款日</span>" +
                    "<span class='clndr-he'>还款/回款日</span>" +
                    "<span class='clndr-cur'>当前日期</span>" +
                "</div>" +
                "<div class='clndr-repay-time'>" +
                    "还款时间：每日00:15" +
                "</div>" +
            "</div>" +
            "<div class='clndr-datebar'>" +
        ...
        "<div class='event-listing'>" +
            "<div class='event-listing-title'>本月事件</div>" +
            "<div class='event-wrap'>" +
            "<% _.each(eventsThisMonth, function(event) { %>" +
                "<div class='event-item'>" +
                    "<div class='event-item-time'><%= event.date %><em><%= event.name %></em></div>" +
                    "<% if(event.name === '回款日'){ %>" +
                        "<div class='event-item-title'><%= event.title %></div>" +
                        "<div class='event-item-num'><%= event.inum %></div>" +
                        "<div class='event-item-method'><%= event.imethod %></div>" +
                    "<% }else if(event.name === '还款日'){ %>" +
                        "<div class='event-item-num'><%= event.bnum %></div>" +
                        "<div class='event-item-method'><%= event.bmethod %></div>" +
                    "<% }else if(event.name === '还款/回款日'){ %>" +
                        "<div class='event-item-title'><%= event.title %></div>" +
                        "<div class='event-item-num'><%= event.inum %></div>" +
                        "<div class='event-item-method'><%= event.imethod %></div>" +
                        "<div class='event-item-gap'></div>" +
                        "<div class='event-item-num'><%= event.bnum %></div>" +
                        "<div class='event-item-method'><%= event.bmethod %></div>" +
                    "<% } %>" +
                "</div>" +
    ...
    Clndr.prototype.render = function () {
        ...
        if (this.options.lengthOfTime.days) {
            ...
            data = {
                ...
                intervalEnd: null,
                intervalStart: null,
                year: this.month.format('YYYY年'),//已改动
                eventsThisInterval: null,
                extras: this.options.extras,
                month: this.month.format('MM月'),//已改动
                daysOfTheWeek: this.daysOfTheWeek,
                eventsLastMonth: this.eventsLastMonth,
                eventsNextMonth: this.eventsNextMonth,
                numberOfRows: Math.ceil(days.length / 7),
